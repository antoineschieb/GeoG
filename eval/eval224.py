...
def evaluate(N, ohe, small_ds):
    ...
    cv2.imwrite(
        f"{ROOTDIR}training/full_img_eval/p{N}_{country_label[0]}_{predicted_label[0]}_{round(confidence, 2)}.png",
        orig_patch,
    )
    return pred
...
if __name__ == "__main__":
    ...
    # base dataset
    ds = PartitionedDataset(f"{DATADIR}v3", 10, (224, 224, 3), offset=(0, 0))
    small_ds = PartitionedDataset(
        f"{DATADIR}v3_eval", 10, (224, 224, 3), offset=(0, 0), ohe=ds.ohe
    )
    # model
    torch.hub.set_dir("./cache-dir/")
    model = models.resnet18(progress=False)
    ...
    model.load_state_dict(
        torch.load(f"{ROOTDIR}training/saved_models/ResNet18/ccub_BEST/latest.pth")
    )
    model.to(device)
    model.eval()
